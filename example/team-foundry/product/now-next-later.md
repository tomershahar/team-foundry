...
- **Exception UI v2** — redesigned exception detail view showing why flagged and what to check. In development (Priya + Leo). Target: ship by May 9.
- **Approval routing by invoice amount** — threshold-based routing so low-risk invoices skip senior approver. In development (Jake). Unblocked.
- **Tolerance configuration audit trail** — show history of changes to auto-approval tolerance per customer. Scoped, starting next sprint (Aino).
