...
The coach activates in three modes:

- **Inline** — during normal AI tool interaction, when the user's question would be measurably better-answered by a file that's empty or stale. The coach speaks briefly, cites the file, offers the next step. It has memory of recent nudges so it doesn't repeat.
- **Explicit** — user types "let's do a team-foundry review" or equivalent. Full audit, all coaching behaviors run.
- **Scheduled** — weekly by default (configurable). Runs the full audit, surfaces top 3 issues, offers to draft fixes for the most important ones.
...
- Has memory of recent nudges to avoid repetition on the same issue within a configurable window.
...
- F4.2 — Three activation modes: **inline** (contextual, when it helps the current question), **explicit** (user asks for review), **scheduled** (weekly audit by default, configurable).
...
- F4.5 — **Nudge memory.** Coach tracks recently-flagged issues and doesn't repeat within a configurable window (default 7 days) unless the user asks.
