...
| Claude Code | `CLAUDE.md` imports `@AGENTS.md` | Documented-only; [Claude Code documents this exact adapter](https://code.claude.com/docs/en/memory#agentsmd) |
| Gemini CLI | `GEMINI.md` imports `@./AGENTS.md` | Documented-only; [Gemini CLI documents Markdown imports](https://geminicli.com/docs/cli/gemini-md/#modularize-context-with-imports) |
| Cursor | Root `AGENTS.md` plus an always-applied rule that imports it via `@AGENTS.md` and instructs reading `AGENTS.md` by name as a fallback | Documented-only; [Cursor documents both root AGENTS.md and project rules](https://docs.cursor.com/context/rules-for-ai). `@file` expansion inside rule body content is not documented — the explicit read instruction is the guaranteed path |
